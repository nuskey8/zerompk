...
fn bench_values(b: &mut test::Bencher, values: Vec<i32>) {
    b.iter(|| {
        for _ in 0..N {
            let encoded = zerompk::to_msgpack_vec(test::black_box(&values)).unwrap();
...
#[bench]
fn serialize_i32_positive_fixint(b: &mut test::Bencher) {
    bench_values(b, (0..ELEMENTS).map(|i| (i % 128) as i32).collect());
}

#[bench]
fn serialize_i32_negative_fixint(b: &mut test::Bencher) {
    bench_values(b, (0..ELEMENTS).map(|i| -1 - (i % 32) as i32).collect());
}

#[bench]
fn serialize_i32_int8(b: &mut test::Bencher) {
    bench_values(b, (0..ELEMENTS).map(|i| -33 - (i % 96) as i32).collect());
}

#[bench]
fn serialize_i32_int16(b: &mut test::Bencher) {
    bench_values(
        b,
        (0..ELEMENTS)
            .map(|i| {
...
#[bench]
fn serialize_i32_int32(b: &mut test::Bencher) {
    bench_values(
        b,
        (0..ELEMENTS)
            .map(|i| {
...
#[bench]
fn serialize_i32_mixed(b: &mut test::Bencher) {
